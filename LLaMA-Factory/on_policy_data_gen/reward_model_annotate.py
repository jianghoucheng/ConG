...
parser.add_argument(
    "--output_dir",
    type=str,
    default="datasets/Qwen2.5-3B-Instruct/",
    help="Path to output directory (will contain generation file, etc.)"
)
parser.add_argument(
    "--reward_model",
    type=str,
...
generation_file = os.path.join(args.output_dir, "all_outputs.json")
...
file_name = os.path.basename(generation_file).split('.json')[0] + "_rm.json"
...
for data in output_data:
    ...
    data.update({"chosen": chosen, "rejected": rejected})

# Save binarized file
output_file = os.path.basename(generation_file).split('.json')[0] + "_bin.json"
