...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        elif generation_mode == GenerationMode.CONTRASTIVE_DECODING:
            if self._is_stateful:
                # DoLa decoding was not designed for stateful models, and would require some changes
                raise ValueError(
                    f"contrastive decoding is not supported with stateful models, such as {self.__class__.__name__}"
                )
            result = self._contrastive_decoding(
                input_ids,
                student_model=generation_config.student_model,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                generation_config=generation_config,
                synced_gpus=synced_gpus,
    ...
    def _contrastive_decoding(
        ...
        **model_kwargs,
        ...
        if self.config.is_encoder_decoder:
            raise ValueError("Contrastive decoding is only available for decoder-only models.")
        # init values
        pad_token_id = generation_config._pad_token_tensor
        ...
        # prepare student_model for Contrastive decoding
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            base_logits = student_outputs.logits[:, -1, :].clone().float()
            final_logits = outputs.logits[:, -1, :].clone().float()
            final_logits, base_logits = _relative_top_filter(final_logits, base_logits, relative_top = relative_top)
            base_logits = base_logits.log_softmax(dim=-1)
            next_token_logits = final_logits - (1-alpha)*base_logits
