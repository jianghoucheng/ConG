...
class GenerationConfig(PushToHubMixin):
    ...
    def __init__(self, **kwargs):
        ...
        self.student_model = kwargs.pop("student_model", None)
        # Parameters that control the cache
    ...
    def get_generation_mode(self, assistant_model: Optional["PreTrainedModel"] = None) -> GenerationMode:
        ...
        if self.student_model is not None:
            if generation_mode in ("greedy_search", "sample"):
                generation_mode = GenerationMode.CONTRASTIVE_DECODING
            else:
                raise ValueError(
                    "You've set `student_model`, which triggers Contrastive Decoding generate. Currently, Contrastive Decoding generate "
                    "is only supported with Greedy Search and Sample."
                )
